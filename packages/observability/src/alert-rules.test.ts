...
describe('Prometheus 规则文件', () => {
  ...
  const SUPPLEMENTARY_ALERTS: Readonly<Record<string, string>> = {
    ...
    TravelCreditWriteOffHigh:
      'CR 结算「不足由我们承担」是刻意的取舍（另两种做法一个要向用户解释负余额、一个要丢掉已经生成好的计划），而它的代价是亏损完全静默：用户照样拿到计划、任务照样 COMPLETED。这条曲线是「现在正在亏多少」的唯一信号',
    TravelCreditUnpriced:
      '运营加模型与配价格在不同的表、由不同的命令完成，忘配一定会发生。而此时系统不报错 —— 命中兜底价就按一个偏贵的价收，连兜底都没有就完全不收费。两种都只有这条曲线能发现',
    TravelCreditInsufficientRatioHigh:
      '产品告警而非故障告警。定价配错时每一次都返回一个语义完全正确的 402，而用户不会来问「为什么我不能生成」——他会走。实测过一次：占位价下 14 天行程的预留额（10578 CR）就超过注册赠送额（9900 CR），刚注册的用户点 14 天必然被拒',
    TravelCreditHoldExpiredHigh:
      '过期回收是兜底，走到它说明那些任务一个都没经过正常结算（Worker 被 OOM/SIGKILL、结算抛异常、或入队失败后无消费者）。而回收本身把钱退了，因此用户不会投诉、任务计数也正常 —— 这条曲线是「有一批生成默默烧掉了模型的钱却没产出」的唯一信号。不用 absent() 是因为该指标只在 CREDIT_BILLING_ENABLED 开启时存在，而默认部署是关的',
    TravelCreditHoldSweepFailing:
      '与上一条是两种不同故障：那条是「回收在工作、上游有问题」，这条是「回收本身坏了」。后者的后果是预留永久冻住：reserve 把 CR 从 balance_cr 搬到 held_cr，没人搬回则用户可用余额一直少那么多，而 credit_holds_job_uk 让那个任务连重新预留都做不到。一轮失败不让进程退出，因此除了这条曲线没有其他症状',
    TravelQueueAdmissionRejecting:
      '与 TravelQueueBacklogHigh 是一对但硬得多：深度高只是「排队变长」，而这条非 0 意味着真实用户拿到了 503。积压告警的 for: 5m 就是在有意容忍突发，而丢请求不能容忍。阈值取 > 0 而不是比例：准入阈值（缺省 40）已远高于告警阈值（8），走到拒绝说明积压堆到了「新任务在开始之前就会撞上 600 秒」的程度，不存在可接受的小比例',
    TravelRenderBrowserRestartHigh:
      'R-84 的自愈把一个刺眼的故障变成了一次重试：以前 Chromium 崩溃以「后续每一个导出都失败」的形式表现，现在只是三个在途导出失败 + 下一个任务重启后正常。自愈是对的，而副作用是崩溃变得不可见 —— 这个指标是它唯一的信号，而在此之前没有人盯它（运维手册那一节开头写的就是「没有告警，靠看指标」）。阈值 > 3/小时按运维手册的分档定：1～2 次是自愈已经接住的偶发，定得再低会变成噪声',
  };
