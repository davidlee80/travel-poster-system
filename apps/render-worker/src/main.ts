...
import { processExportJob, recoverExports } from './run-export.js';
...
const execution = createExportExecutionRepository(dbPool, billingEnabled);
...
await runWorker({
  ...
  start: async (handle) => {
    ...
    const recoveryQueue = new Queue(EXPORT_QUEUE_NAME, { connection: queueRedis });
    ...
    const recover = (): void => {
      ...
      recoveryRunning = (async () => {
        const failedJobs: { exportId: string; errorCode: string }[] = [];
        try {
          ...
          for (const job of jobs) {
            ...
            if (parsed.success && (await job.getState()) === 'failed') {
              failedJobs.push({ exportId: parsed.data.exportId, errorCode: job.failedReason });
            }
          }
        } catch (error) {
          handle.logger.warn({ err: error }, '导出失败队列扫描失败，下轮继续');
        }
        await recoverExports({ execution, storage, logger: handle.logger, failedJobs });
      })()
        .catch((error: unknown) => {
          handle.logger.warn({ err: error }, '导出恢复扫描失败，下轮继续');
        })
        .finally(() => {
