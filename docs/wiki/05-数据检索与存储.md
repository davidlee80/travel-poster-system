...
| `0016_template_suite_rename`         | `template_id` 正名为样式套件 ID（R-85 P1）                                   | `TEMPLATE_REGISTRY`、presentation 读路径按 `template_id` 过滤 |
| `0017_template_catalog`              | 样式套件目录（多模板选择，R-85 P3）                                          | 前端模板选择器、素材加载                                      |
| `0018_export_retry_after_failure`    | exports 幂等唯一索引排除 `FAILED` 行，失败可同键重试                         | exports 路由、render-worker 退款路径                          |
| `0019_idempotency_result_window`     | 幂等结果复用窗口（7 天）落地：`superseded_at` + 部分唯一索引                 | `db/travel-plans.ts`、生成端点幂等分支                        |
| `0020_planner_field_requirements`    | 76 字段的发布版必填分类与条件触发器（`field_requirements` JSONB 随版本克隆） | planner config、字段校验                                      |
