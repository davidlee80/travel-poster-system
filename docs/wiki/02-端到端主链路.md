...
sequenceDiagram
  ...
  A->>A: IP 限流、队列深度准入、身份（匿名 403 拦截）、Schema、标准化、N规则、配额/CR报价、灰度
  A->>D: 计费开启时原子预留 CR
...
| 步骤                  | 实现位置                                                       | 失败表现                                    |
| --------------------- | -------------------------------------------------------------- | ------------------------------------------- |
| 每 IP 限流            | `server.ts` 注册的 `@fastify/rate-limit`（默认 300/IP/分钟）   | 429 `SYS_RATE_LIMITED` + `Retry-After`      |
| 队列深度准入          | `queue-depth.ts` `decideAdmission`（深度 ≥ 40 拒收）           | 503 `SYS_QUEUE_SATURATED` + `Retry-After`   |
| 身份解析              | `routes/identity-context.ts`、`identity/service.ts`            | 无会话时现场建号（保留回切路径）            |
| 匿名生成拦截          | `travel-plans.ts` 生成端点                                     | 匿名身份一律 403 `AUTH_ANONYMOUS_FORBIDDEN` |
...
两个准入闸门的顺序是刻意的：每 IP 限流由 Fastify 全局钩子完成（内存计数，
探针与 `/metrics` 豁免），早于一切业务逻辑；队列深度准入在**身份解析之前**——
否则过载时每个被拒的访客都会先留下一行 `users` 与一个会话，那正是背压要省下的写入。
队列深度采自 BullMQ 真实深度，阈值 40 从 600 秒排队上限与单副本吞吐反推
（`queue-depth.ts` 有完整推导）；采样不可判定时放行（fail open）。
...
2. `exports.idempotency_key` 保证重复申请返回原 `export_id`（迁移 0018 起该唯一索引排除 `FAILED` 行，失败后可同键重试；0019 起结果复用有 `IDEMPOTENCY_RESULT_TTL_DAYS`=7 天窗口，超窗即新意图）；
3. 计费开启时按 PNG/PDF 固定 SKU 在建行成功后扣 CR（扣费在建行之后：并发竞争由唯一索引定胜负，输家一分钱都没扣）；消息进入独立 `travel-plan-export` 队列，**入队失败立即退款 + 行置 `FAILED`**（退款键 `refund:export:<export_id>`，与 render-worker 侧一致）；
...
9. 预留兜底：`credit_holds.expires_at`（2 小时）由 retention-worker 的独立 15 分钟
   `hold-sweep` 定时器扫描回收（`apps/retention-worker/src/hold-sweep.ts`），
   覆盖结算 catch、SIGKILL/OOM、入队失败等所有泄漏入口；指标
