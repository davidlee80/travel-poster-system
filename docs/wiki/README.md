...
9. [前端实现](09-前端实现.md)：全部页面与九步问卷的实现细节、组件索引和修改入口。

上面 01–08 八篇负责**广度**，每篇刻意压在 100 行上下；09 是前端实现的**深度索引**。单个子系统的**具体阈值与判据**
...
| 排查生成失败         | [端到端主链路](02-端到端主链路.md)、[生成链路时序与判据](专项说明/生成链路时序与判据.md) | [`apps/generation-worker/src/generate-plan.ts`](../../apps/generation-worker/src/generate-plan.ts)                                                                                                             |
